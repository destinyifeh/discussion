...
function Home() {
  ...
  useEffect(() => {
    axios
      ...
      .then((res) => {
        ...
        console.log(res.data.loginUsername);

        let dot = res.data.filter((loginUser) => loginUser.loginUsername);
  ...
  let dotUser = Array.from(dot);
  let check = dotUser.map((user) => user.loginUsername);
    ...
    <>
      ...
      <div className="container-fluid">
        ...
        <p className="text-center">
          ...
          <span className="">Time: {dayjs().format("h:ma")}</span>
        </p>
        ...
        <div className="container profile my-4">
          <div className="d-flex justify-content-center">
            ...
            <form
              className="my-2 home-form d-none d-lg-inline"
              onSubmit={onSubmit}
            >
              <div className="input-group">
                <input
                  type="search"
                  className="form-control lg-search shadow-none"
                  placeholder="Search..."
                  value={query}
                  ...
                  <div className="card mb-3" key={feature._id}>
                    <div className="p-2">
                      <span className=" p-2">
                        ...
                        <Link
                          className="card-name"
                          to={`/user/${feature.username}`}
                        >
                          <img className="image " src={feature.photo} alt="" />{" "}
                          {feature.username}
                        </Link>{" "}
                        <span className="status">
                          {check.includes(feature.username) ? (
                            <i className="fa fa-circle text-success circle"></i>
                          ) : (
                            <i className="fa fa-circle circle2"></i>
                          )}{" "}
                      ...
                      <i className="fa fa-clock-o text-dark"></i>{" "}
                      {dayjs(feature.createdAt).fromNow()}
                      <Link className="" to={`/feature/${feature.slug}`}>
                        <h5 className="mt-1 feature_title">{feature.title} </h5>
                      </Link>
                    ...
                    <div className="d-flex justify-content-between">
                      ...
                      <div className="d-flex justify-content-even p-2">
                        <Link
                          className=""
                          to={`/report/feature/${feature.title}`}
                        >
                          report <i className="fa fa-angle-double-right"></i>
                        </Link>
                      </div>
                    </div>
                        ...
                        <div className=" border-bottom" key={online._id}>
                          <span className=" p-2">
                            ...
                            <Link
                              ...
                              to={`/user/${online.loginUsername}`}
                            >
                              <img
                                className="images  img-thumbnail d-block mx-auto"
                                src={online.photo}
                                alt=" Profile"
                              />{" "}
                            </Link>
